  ...
  m_block_udp_status_updates {false}
{
  ...
  connect (m_messageClient, &MessageClient::setup_tx, [this] (int newTxMsgIdx, QString const& msg, bool param0, bool param1, QString const& check, quint32 offset) {    //avt 11/16/20
      if (!m_externalCtrl) {
        ...
        showStatusMessage("Controller initiated"); //avt 10/4/25
        debugToFile(QString{});
      ...
      if (newTxMsgIdx) {          //perform some QSO state action (mostly)
        ...
        if (newTxMsgIdx == 6 || newTxMsgIdx == 4) {   //set up some kind of CQ, set options
          ...
          debugToFile(QString{"setupCq      statusUpdate, m_auto:%1 autoButton isChecked:%2"}.arg(m_auto).arg(ui->autoButton->isChecked()));
        ...
        if (newTxMsgIdx == 5) {   
          m_debugLog = true;
          debugToFile(QString{});
          debugToFile("setup        Logging enabled");
          return;
        }
        ...
        if (newTxMsgIdx == 8) {           //disable Tx
          debugToFile(QString{});
          debugToFile("disableTx");
          m_enableButtonNotify = false; //avt 1/23/24
          process_autoButton(false);
          ui->autoButton->setChecked(param0);
          m_enableButtonNotify = true; //avt 1/23/24
          debugToFile(QString{"disableTx(2) m_auto:%1 autoButton isChecked:%2"}.arg(m_auto).arg(ui->autoButton->isChecked()));
          return;
        }

        if (newTxMsgIdx == 9) {           //enable Tx
          debugToFile(QString{});
          debugToFile("enable Tx");
          m_enableButtonNotify = false; //avt 1/23/24
          process_autoButton(true);
          ui->autoButton->setChecked(param0);
          m_enableButtonNotify = true; //avt 1/23/24
          debugToFile(QString{"enableTx(2)  m_auto:%1 autoButton isChecked:%2"}.arg(m_auto).arg(ui->autoButton->isChecked()));
          return;
        }
        ...
        if (newTxMsgIdx == 12) {          //avt 12/31/21 halt tx
          ...
          m_enableButtonNotify = true; //avt 1/23/24
          debugToFile(QString{"haltTx(2)    m_auto:%1 autoButton isChecked:%2"}.arg(m_auto).arg(ui->autoButton->isChecked()));
          return;                         //avt 1/25/22
        }
      ...
      else      //special cmd
      {
        ...
        initExternalCtrl();
        debugToFile(QString{});
        debugToFile("setup        Controller terminated");
        m_debugLog = false;
  ...
  setWindowTitle (program_title () + "   (mod)");
  ...
  //testing# included:
  debugToFile(QString{"41           data_dir:'%1' writeable_data_dir:'%2'"}.arg(m_config.data_dir().absolutePath()).arg(m_config.writeable_data_dir().absolutePath()));
...
void MainWindow::closeEvent(QCloseEvent * e)
{
  ...
  m_debugLog = false;   //temporarily stop debug logging
...
//Right (Rx Frequency) window
      ...
      if(m_mode=="FT8" or m_mode=="FT4" or m_mode=="FST4" or m_mode=="Q65" or m_mode=="JT65" or m_mode=="JT9") {
        int ftol=10;
...
//### I think this is where we are preventing Hounds from spotting Fox ###
      if(m_mode!="FT8" or (SpecOp::HOUND != m_specOp) or (SpecOp::HOUND == m_specOp and m_config.superFox())) {
        if(m_mode=="FT8" or m_mode=="FT4" or m_mode=="Q65"
           ...
           or m_mode=="JT65"  //avt 1/6/24
           or m_mode=="JT9") {   //avt 1/7/24
...
//----------------------------------------------------------------------
    ...
    if (m_sentFirst73 || (is_73 && CALLING == m_QSOProgress)) {
      ...
      if((m_config.prompt_to_log() or m_config.autoLog()) && !m_tune && CALLING != m_QSOProgress)
        {
          if (!is_externalCtrlMode()) logQSOTimer.start(0);   //avt
        }
      else
        {
    ...
    //                                                                                                  //avt 1/6/24         //avt 1/7/24
    bool b=("FT8"==m_mode or "FT4"==m_mode or "Q65"==m_mode or m_mode == "FST4" or m_mode == "MSK144" or m_mode == "JT65" or m_mode == "JT9")
         and ui->cbAutoSeq->isVisible () and (ui->cbAutoSeq->isEnabled () or is_externalCtrlMode()) && ui->cbAutoSeq->isChecked ();    //avt
...
//        });
  ...
  if (!(modifiers & Qt::ShiftModifier) && (modifiers & Qt::AltModifier)) {    //avt 1/1/21 detect alt/dbl-click and ctrl/alt/dbl-click (but not shift/dbl-click)
    ...
    if (((QDateTime::currentMSecsSinceEpoch() / 1000 - m_secBandChanged) > 4*int(m_TRperiod)/4)) {  //band wasn't recently changed
      //debugToFile(QString {"doubleClick: call:%1 m_mode:%2 grid:%3 m_currentBand:%4"}.arg(call).arg(m_mode).arg(grid).arg(m_currentBand));
    ...
    {
      debugToFile("doubleClick  band is invalid " + call); 
...
void MainWindow::processMessage (DecodedText const& message, Qt::KeyboardModifiers modifiers)
{
  ...
  if (!m_externalCtrl and !is_73 and !message.isStandardMessage() and !message.clean_string ().contains("<")) {   //avt 1/21/24
        ...
        if (("RRR" == word_3
          ...
          if(SpecOp::RTTY == m_specOp and m_nextCall!="") {
            ...
            if (m_config.prompt_to_log() || m_config.autoLog()) {
              ...
              //debugToFile("processMsg:  cease tx(2)");
              cease_auto_Tx_after_QSO ();
            }
            ...
            if (false)              // Always Send 73 after receiving RRR or RR73, even in contest mode.
              {
                if (m_config.prompt_to_log() || m_config.autoLog()) {
                  ...
                  //debugToFile("processMsg:  cease tx(3)");
                  cease_auto_Tx_after_QSO ();
                }
                m_ntx=6;
                ui->txrb6->setChecked(true);
                            ...
                            , QString const& propmode, QByteArray const& ADIF)
{
  ...
  //avt 11/20/20 UDP listener needs UI feedback
  m_messageClient->logged_ADIF (ADIF);

  logIncremental(call, QString::fromUtf8(ADIF));
...
void MainWindow::on_stopTxButton_clicked()                    //Stop Tx
{
  ...
  if (m_tune) stop_tuning ();
  if ((ui->autoButton->isChecked() or m_auto) and !m_tuneup) auto_tx_mode (false);  //avt 1/24/24
...
// In auto-sequencing mode, stop after 5 transmissions of "73" message.
  if (m_bFastMode || m_bFast9) {
    if (ui->cbAutoSeq->isVisible () && (ui->cbAutoSeq->isEnabled () or is_externalCtrlMode()) && ui->cbAutoSeq->isChecked ()) {    //avt
      if(m_ntx==5) {
        m_nTx73 += 1;
      } else {
        m_nTx73=0;
                            ...
                            , bool useReply, quint8 modifiers)        //avt 1/1/21
{
  ...
  if (!cursor.isNull () || is_externalCtrlMode()) //avt 11/28/20
    {
      if (!is_externalCtrlMode()) {      //avt 11/20/20 UDP listener doesn't need user attention
...
void MainWindow::postDecode (bool is_new, DecodedText decoded_text)      //avt 12/5/20
{
  ...
  if (parts.size () >= 5 && (!is_externalCtrlMode() || decoded_text.isStandardMessage()))
    {
      //QApplication::beep();
...
void MainWindow::statusUpdate () const
{
  ...
  auto tr_period = (int)(ui->sbTR->value () * 1000);      //avt 11/14/21
  auto rx_frequency = ui->RxFreqSpinBox->value ();
  if ("FST4W" == m_mode)
    {
      tr_period = (int)(ui->sbTR_FST4W->value () * 1000);     //avt 11/14/21
...
void MainWindow::tx_watchdog (bool triggered)
{
  ...
  if (triggered)
    {
      if (m_auto && m_mode == "MSK144")         //avt 12/12/21 no external ctrl for MSK144
      {
        m_bCallingCQ = true;        // allows tail-enders to be picked up
        m_ntx=6;
        ui->txrb6->setChecked(true);
...
void MainWindow::on_cbAutoSeq_toggled(bool b)
{
  ...
  ui->respondComboBox->setVisible((m_mode=="FT8" or m_mode=="FT4" or m_mode=="FST4" or m_mode == "MSK144" or m_mode=="Q65") and b);	//avt
}
...
void MainWindow::on_actionDownload_from_LOTW_triggered()
{
  ...
  if ((ui->autoButton->isChecked()) and MessageBox::No == MessageBox::query_message (this,
